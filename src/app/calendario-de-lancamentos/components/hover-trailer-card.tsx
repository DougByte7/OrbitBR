...
export default function HoverTrailerCard({ anime }: HoverTrailerCardProps) {
  const [isMobile] = useState(mobileCheck);
  const Container = isMobile ? Popover : HoverCard;
  const Trigger = isMobile ? PopoverTrigger : HoverCardTrigger;
  const Content = isMobile ? PopoverContent : HoverCardContent;
  const playerSize = isMobile ? 300 : 560;
  ...
  const streamings = Array.from(
    new Map(
      anime.streamingEpisodes
        .map(({ site, url }) => {
          const name = url.match(/www\.(?<s>[^.]+)\.com/)?.groups?.s;
          return name ? { site, url, name } : null;
        })
        .filter(
          (s): s is { site: string; url: string; name: string } => s !== null,
        )
        .map((s) => [s.site, s] as const),
    ).values(),
  );

  return (
    <Container>
      <Trigger className="grid justify-items-center gap-2">
        <img
        ...
        <div className="flex flex-wrap gap-2">
          {streamings.map(({ site, url, name }) => (
            <a key={site} className="w-fit" href={url} target="_blank">
              <img
                className={clsx(
      ...
      <Content side={isMobile ? "bottom" : "right"}>
        ...
        >
          ...
          {!!anime.episodes && <p>{anime.episodes} episódios</p>}
        </div>
      </Content>
    </Container>
  );
